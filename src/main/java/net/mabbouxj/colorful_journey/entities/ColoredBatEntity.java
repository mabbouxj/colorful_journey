...
public class ColoredBatEntity extends BatEntity implements IColoredMobEntity {

    private static final DataParameter<Integer> DATA_COLOR_ID = EntityDataManager.defineId(ColoredBatEntity.class, DataSerializers.INT);
    private final ResourceLocation LAYER_LOCATION = new ResourceLocation(ColorfulJourney.MOD_ID, "textures/entity/bat/colored_bat_layer.png");

    public ColoredBatEntity(EntityType<? extends BatEntity> entityType, World world) {
        super(entityType, world);
        this.setColor(ColorUtils.getRandomEnabledColor());
    }

    public ColoredBatEntity(World world, BatEntity oldEntity, DyeColor color) {
        this(ModEntityTypes.COLORED_BAT.get(color).get(), world);
    ...
    public static AttributeModifierMap.MutableAttribute createAttributes(DyeColor color) {
        return MobEntity.createMobAttributes()
                .add(Attributes.MAX_HEALTH, 6.0D * ColorAttributesModifier.HEALTH.byColor(color));
    }
    ...
    public ResourceLocation getDefaultLootTable() {
        return new ResourceLocation(ColorfulJourney.MOD_ID, "entities/colored_bat");
    }
