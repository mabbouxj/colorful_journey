package net.mabbouxj.colorful_journey.utils.nbthandler;

import com.google.common.collect.Lists;
import net.mabbouxj.colorful_journey.annotations.Sync;
import net.mabbouxj.colorful_journey.utils.nbthandler.data.*;
...
public class NBTManager {
    ...
    private static final NBTManager ourInstance = new NBTManager();
    private final List<INBTHandler> handlerList;
    private final HashMap<Class<? extends TileEntity>, List<Field>> tileFieldList;
    ...
    /**
     * Scans a {@link TileEntity} class for {@link Sync}.
     *
    ...
    public void scanTileClassForAnnotations(Class<? extends TileEntity> entity) {
        ...
        for (Field field : getAllDeclaredFields(entity)) {
            if (field.isAnnotationPresent(Sync.class) && checkForHandler(field)) {
    ...
    /**
     * Writes all the values that have {@link Sync} annotation to the NBTTagCompound.
     *
    ...
    public CompoundNBT writeTileEntity(TileEntity entity, CompoundNBT compound) {
        if (tileFieldList.containsKey(entity.getClass())) {
            for (Field field : tileFieldList.get(entity.getClass())) {
                Sync save = field.getAnnotation(Sync.class);
                try {
